...
int bc, ec ;
long cksum ;
float efactor = 1.0, slant = 0.0, letterspacing = 0.0;
float capheight = 0.8 ;
char *efactorparam = NULL, *slantparam = NULL;
double newslant ;
char titlebuf[500] ;
...
writevpl P1H(void)
{
   ...
   voutln2("(SPACE R %.1f)", fontspace + 4.0 * letterspacing) ;
   if (! fixedpitch) {
   ...
   for (i=bc; i<=ec; i++)
      if (0 != (ai=texptrs[i])) {
         ...
         if (uppercase[i]) {
            ai=uppercase[i] ;
            voutln2("(CHARWD R %.1f)", capheight * (ai->width) + 2.0 * letterspacing) ;
            ...
            voutln2("(CHARWD R %.1f)", ai->width + 2.0 * letterspacing) ;
            if (0 != (ht=texheight(ai)))
         ...
         if (ai->adobenum != i || uppercase[i] || letterspacing != 0.0) {
            vleft() ; voutln("MAP") ;
            if (letterspacing != 0.0) voutln2("(MOVERIGHT R %.1f)", letterspacing);
            ...
            if (letterspacing != 0.0) voutln2("(MOVERIGHT R %f)", letterspacing);
            vright() ;
         }
         vright() ;
      }
...
void version P1C(FILE *, f)
{
  ...
  fputs ("afm2tfm(k) (dvips(k) 5.92a) 8.1\n", f);
...
-l REAL             increase sidebearings by REAL.\n\
...
void usage P1C(FILE *, f)
{
 ...
 "Usage: afm2tfm foo[.afm] [-O] [-u] [-v|-V bar[.vpl]]\n") ;
   (void)fprintf(f,
 "                 [-e expansion] [-l letterspacing] [-s slant] [-c capheight]\n") ;
   (void)fprintf(f,
...
case 'l':
         CHECKARG3
         if (sscanf(argv[3], "%f", &letterspacing)==0)
